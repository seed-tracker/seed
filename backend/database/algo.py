...
def find_correlations(username):
    ...
    symptom_counts = list(filter(lambda symptom: symptom['count'] >= 50, [sym for sym in db.user_symptoms.aggregate(symptom_pipeline)]))
    ...
    print(pd.DataFrame([list(s.values()) for s in symptom_counts], columns=['count', 'avg_severity', 'symptom']))
...
def create_df(data):
    ...
    data_frame = []
    for i in data:
        data_frame.append([i[0], i[1]['total_count'], i[1]['lift'], i[1]['severity_avg']])
...
def check_data(item_sets, symptom_name, min_threshold):
    ...
    sorted_list = list(filter(lambda y: y[1]['overlap'] > min_threshold and y[1]['overlap']/y[1]['total'] > 0.7, sorted(item_count.items(), key=lambda x: -x[1]['overlap'])))[:10]
